...
file_path = "/csp_haplotype_monthly_frequency_for_human_mosquito_head.csv"
output_dir = "/figures/S2_fig.tif"
...
df = df[df["month_year"].isin(valid_months)]o
...
def plot_haplotypes(data, hap_subset, panel_tag, filename):
    ...
    ax.set_ylabel("csp haplotype", fontsize=AXIS_FONT)
...
plot_haplotypes(df, haplotype_groups[0], "A", "FigS2A.tif")
plot_haplotypes(df, haplotype_groups[1], "B", "FigS2B.tif")
plot_haplotypes(df, haplotype_groups[2], "C", "FigS2C.tif")
plot_haplotypes(df, haplotype_groups[3], "D", "FigS2D.tif")